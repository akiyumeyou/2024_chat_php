...
$text_size_80_percent = $text_size * 0.8;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['image'])) {
    ...
    if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadedFile)) {
        ...
        $x = $width / 2;
        $y = calculateYPosition($height, $text_position, $text_size_80_percent);

        list($left, , $right) = imageftbbox($text_size_80_percent, 0, $fontPath, $text_overlay);
        $textWidth = $right - $left;
        $x -= $textWidth / 2;

        if (!imagettftext($resizedImage, $text_size_80_percent, 0, $x, $y, $color, $fontPath, $text_overlay)) {
            die("テキストの追加に失敗しました。");
        }
